...
export class AppComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  private initMap() {
    ...
    this.map.on('click', (e: L.LeafletMouseEvent) => {
      ...
      const clickResult = this.addClickCoins();
      this.showCoin(e.originalEvent as MouseEvent, clickResult);
    });
...
`);
  ...
  private addClickCoins() {
    const clickResult = this.calculateClickEarned();
    const earned = clickResult.earned;
  ...
  private showCoin(
    event: MouseEvent,
    clickResult: { earned: number; isCritical: boolean },
  ) {
    const { earned, isCritical } = clickResult;
